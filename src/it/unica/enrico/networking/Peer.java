...
import it.unica.enrico.models.Block;
import it.unica.enrico.models.Blockchain;
import it.unica.enrico.models.Data;
import it.unica.enrico.models.Transaction;
...
public abstract class Peer {
    ...
    private final MessageListener listener                      = new MessageListener() {
        ...
        public void onMessage(Receiver recv) {
            ...
            while (data != null) {
                final String from = data.getMittente();

                final byte[] bytes = data.getMessaggio().array();
    ...
    public boolean isReady() {
        return (runnableRecvTcp.isReady() && runnableSendTcp.isReady() &&
                runnableRecvMulti.isReady() && runnableSendMulti.isReady());
    }
    ...
    private void sendWhois(String who) {
        ...
        final Data data = new Data(myName, runnableRecvTcp.getHost(), runnableRecvTcp.getPort(),
                                   EVERY_ONE, runnableRecvMulti.getHost(), runnableRecvMulti.getPort(),
                                   NO_SIG, msg);
        if (DEBUG) {
            final String string = new String(msg);
            final String hdr = string.substring(0, HEADER_LENGTH);
            System.out.println(myName+" Invio '"+hdr+"' msg");
        }
    ...
    private void handleWhois(byte[] bytes, Data data) {
        ...
        if (name.equals(this.myName) && !(data.getMittente().equals(this.myName)))
            sendIam();
    }
    ...
    private void handleIam(byte[] bytes, Data data) {
        final String name = data.getMittente();
        final byte[] key = parseIamMsg(bytes);

        // Update peers
        peers.put(name, new Host(data.getIndirizzoSorgente().getHostAddress(), data.getPortaSorgente()));
    ...
    private Constants.Status handleTransaction(String dataFrom, Transaction transaction, Data data) {
        ...
        if (transaction.getMittente().equals(myName))
            return Constants.Status.OWN_TRANSACTION;

        // Let the app logic do what it needs to
        final Constants.Status status = handleTransaction(dataFrom, transaction, data.getFirma().array(), data.getMessaggio().array());
    ...
    private void handleTransactionAck(String dataFrom, Transaction transaction, Data data) {
        // Let the app logic do what it needs to
        Constants.Status knownPublicKey = handleTransactionAck(dataFrom, transaction, data.getFirma().array(), data.getMessaggio().array());
    ...
    protected void sendBlock(Block block, Data data) {
        ...
        final byte[] sig = signMsg(msg);
        final Data dataToSend = new Data(myName, runnableRecvTcp.getHost(), runnableRecvTcp.getPort(), data.getMittente(), data.getIndirizzoSorgente().getHostAddress(), data
                .getPortaSorgente(), sig, msg);
        if (DEBUG) {
            final String string = new String(msg);
            final String hdr = string.substring(0, HEADER_LENGTH);
            System.out.println(myName+" Sending '"+hdr+"' msg. confirmed="+block.getBloccoConf());
        }
    ...
    private void handleBlock(String dataFrom, Block block, Data data) {
        ...
        if (block.getProvenienza().equals(myName))
            return;

        final int length = this.getBlockChain().getLength();
        final Constants.Status status = checkTransaction(data.getMittente(), block, data.getFirma().array(), data.getMessaggio().array());
        if (status == Constants.Status.NO_PUBLIC_KEY) {
    ...
    protected void sendConfirmation(Block block) {
        ...
        final byte[] sig = signMsg(msg);
        final Data data = new Data(myName, runnableRecvTcp.getHost(), runnableRecvTcp.getPort(), EVERY_ONE, runnableRecvMulti.getHost(),
                                    runnableRecvMulti.getPort(), sig, msg);
        if (DEBUG) {
            final String string = new String(msg);
            final String hdr = string.substring(0, HEADER_LENGTH);
            System.out.println(myName+" Sending multicast '"+hdr+"' msg. confirmed="+block.getBloccoConf());
        }
        sendMultiQueue.add(data);
    }
    ...
    protected void sendConfirmation(Block block, Data data) {
        final byte[] msg = getConfirmationMsg(block);
        final byte[] sig = signMsg(msg);
        final Data dataToSend = new Data(myName, runnableRecvTcp.getHost(), runnableRecvTcp.getPort(), data.getMittente(),
                data.getIndirizzoSorgente().getHostAddress(), data.getPortaSorgente(), sig, msg);
        if (DEBUG) {
            final String string = new String(msg);
            final String hdr = string.substring(0, HEADER_LENGTH);
            System.out.println(myName+" Sending unicast '"+hdr+"' msg. confirmed="+block.getBloccoConf());
        }
        sendTcpQueue.add(dataToSend);
    }
    ...
    private void handleConfirmation(String dataFrom, Block block, Data data) {
        if (block.getBloccoConf()) {
            // Yey! we got a confirmation from the community

            final int length = this.getBlockChain().getLength();
            final Constants.Status status = handleConfirmation(dataFrom, block, data.getFirma().array(), data.getMessaggio().array());
            if (status == Constants.Status.SUCCESS) {
                // Someone solved the nonce, cancel async task - if we have one
                final String hex = HashUtils.bytesToHex(block.getHash());
        ...
        final int length = this.getBlockChain().getLength();
        final Constants.Status status = checkTransaction(dataFrom, block, data.getFirma().array(), data.getMessaggio().array());
        if (status == Constants.Status.NO_PUBLIC_KEY) {
            addBlockToRecv(Queued.State.CONFIRM, dataFrom, block, data);
            sendWhois(dataFrom);
            return;
    ...
    public static class MiningTask extends TimerTask {
        ...
        public void run() {
            ...
            final int nonce = peer.mineHash(this, block.getHash(), block.getNumeroDiZeri());

            if (nonce < 0)
                return;

            // Hash looks good to me and I have computed a nonce, let everyone know
            block.setBloccoConf(true);
